...
public class Tessellation {
    public Shape shape;
    public Map<Transformation, List<RelativeRule>> virtualNeighbourMap = new HashMap<>();
    public Map<Transformation, Vector> offsetVectors = new HashMap<>();
    public Vector basisVector1;
    public Vector basisVector2;
    public HashGraph<Point,Transformation,Boolean> hashGraph = new HashGraph<>();
    ...
    public Tessellation(Shape shape, List<RelativeRule> rules) {
        ...
        Map<Transformation, List<RelativeRule>> relativeRuleMap = calculateRelativeRuleMap(rules);
        System.out.println(relativeRuleMap);
        Map<Transformation, Set<Point>> examplePointsMap = generateMoreExamplePoints(relativeRuleMap);
    ...
    private Map<Transformation, List<RelativeRule>> calculateRelativeRuleMap(List<RelativeRule> rules) {
        Map<Transformation, List<RelativeRule>> relativeRuleMap = new HashMap<>();
        relativeRuleMap.put(Transformation.IDENTITY, rules);
        for (Transformation sym : rules.stream().map(r -> r.symmetry).distinct().toList()) {
            updateRelativeRuleMap(sym, relativeRuleMap);
        }
        return relativeRuleMap;
    }

    private void updateRelativeRuleMap(Transformation symmetry, Map<Transformation, List<RelativeRule>> relativeRuleMap) {
        if(relativeRuleMap.containsKey(symmetry)) return;
        relativeRuleMap.put(symmetry, relativeRuleMap.get(Transformation.IDENTITY).stream().map(r -> r.transform(symmetry)).toList());
    }
    //creating the example points for each symmetry
    private Map<Transformation, Set<Point>> generateMoreExamplePoints(Map<Transformation, List<RelativeRule>> relativeRuleMap) {
        Map<Transformation, Set<Point>> examplePointsMap = new HashMap<>();
        relativeRuleMap.keySet().forEach(s -> examplePointsMap.put(s, new HashSet<>()));
        for (RelativeRule firstRule : relativeRuleMap.get(Transformation.IDENTITY)) {
    ...
    private void calculateOffsetVectors(Map<Transformation, Set<Point>> examplePointsMap) {
        for(Transformation sym : examplePointsMap.keySet()) {
    ...
    private Set<Point> getCenteredPoints(Map<Transformation, Set<Point>> examplePointsMap) {
    ...
    private void deriveBasisVectors(Set<Point> centeredPoints) {
        ...
        Point secondBasisEstimate = basisVector1.toPoint().transform(Transformation.ROT_90);
    ...
    public Point realToVirtual(Point point, Transformation symmetry) {
        Vector offset = offsetVectors.get(symmetry);
        return point.sub(offset).matrixMultiplyInverse(basisVector1, basisVector2);
    }

    public Point virtualToReal(Point point, Transformation symmetry) {
        Vector offset = offsetVectors.get(symmetry);
        return point.matrixMultiply(basisVector1, basisVector2).add(offset);
    } 

    public void generateVirtualNeighbourMap(Map<Transformation, List<RelativeRule>> relativeRuleMap) {
        for (Transformation mainSymmetry : relativeRuleMap.keySet()) {
    ...
    public void findBetterOffsetVectors(Map<Transformation, Set<Point>> examplePointsMap) {
        Predicate<Point> centerParallelogram = p -> realToVirtual(p, Transformation.IDENTITY).equals(Point.ORIGIN);
        for (Transformation symmetry : examplePointsMap.keySet()) {
    ...
    public Point shapeCenter(HashGraph<Point,Transformation,Boolean>.Cluster.Node node) {
    ...
    public void setupHashGraph() {
        ...
        var node = hashGraph.put(new Point(0,0), Transformation.IDENTITY, true);
        for(int i=0;i<20;i++) {
    ...
    public void drawToPlane(Plane plane) {
        ...
        for (var node : hashGraph.traverse(hashGraph.get(Point.ORIGIN, Transformation.IDENTITY))) {
    ...
    public boolean placeShape(Transformation symmetry, Point center, Color color, Plane plane) {
    ...
    public static final Tessellation SMALL_L_2 = new TessellationSetup(Shape.SMALL_L_SHAPE,
        new Pair<>(Transformation.IDENTITY, new Point(2, 0)),
        new Pair<>(Transformation.IDENTITY, new Point(-2, 0)),
        new Pair<>(Transformation.ROT_90, new Point(0, -1)),
        new Pair<>(Transformation.ROT_90, new Point(2, -1)),
        new Pair<>(Transformation.ROT_90, new Point(0, 2)),
        new Pair<>(Transformation.ROT_90, new Point(2, 2)))
        .toTessellation();

    public static final Tessellation SQUARE_1 = new TessellationSetup(Shape.SQUARE,
        new Pair<>(Transformation.IDENTITY, new Point(2, 0)),
        new Pair<>(Transformation.IDENTITY, new Point(-2, 0)),
        new Pair<>(Transformation.IDENTITY, new Point(0, -2)),
        new Pair<>(Transformation.IDENTITY, new Point(0, 2))).toTessellation();

    public static final Tessellation JAGGED_4 = new TessellationSetup(Shape.JAGGED,
        new Pair<>(Transformation.IDENTITY, new Point(0, 2)),
        new Pair<>(Transformation.IDENTITY, new Point(2, 0)),
        new Pair<>(Transformation.ROT_180, new Point(-1, 0)),
        new Pair<>(Transformation.ROT_180, new Point(1, 0)),
        new Pair<>(Transformation.ROT_180, new Point(-1, 2)),
        new Pair<>(Transformation.ROT_180, new Point(4, 5))).toTessellation();

    public static final Tessellation BOWL_6 = new TessellationSetup(Shape.BOWL,
        new Pair<>(Transformation.IDENTITY, new Point(8, 0)),
        new Pair<>(Transformation.IDENTITY, new Point(-8, 0)),
        new Pair<>(Transformation.ROT_180, new Point(3, 0)),
        new Pair<>(Transformation.ROT_180, new Point(11, 0)),
        new Pair<>(Transformation.ROT_180, new Point(3, 5)),
        new Pair<>(Transformation.ROT_180, new Point(11, 5))).toTessellation();
    ...
    public static final Tessellation DOMINO_5 = new TessellationSetup(Shape.DOMINO,
        ...
        new Pair<>(Transformation.ROT_90, new Point(0, -2)),
        new Pair<>(Transformation.ROT_90, new Point(1, -2))).toTessellation();

    public static final Tessellation DOMINO_4 = new TessellationSetup(Shape.DOMINO,
        new Pair<>(Transformation.IDENTITY, new Point(-2, 0)),
        new Pair<>(Transformation.IDENTITY, new Point(2, 0)),
        new Pair<>(Transformation.IDENTITY, new Point(0, -1)),
        new Pair<>(Transformation.IDENTITY, new Point(0, 1))).toTessellation();

        public static final Tessellation DOMINO_6_STRAIGHT = new TessellationSetup(Shape.DOMINO,
        new Pair<>(Transformation.IDENTITY, new Point(-2, 0)),
        new Pair<>(Transformation.IDENTITY, new Point(2, 0)),
        new Pair<>(Transformation.IDENTITY, new Point(-1, -1)),
        new Pair<>(Transformation.IDENTITY, new Point(1, -1)),
        new Pair<>(Transformation.IDENTITY, new Point(-1, 1)),
        new Pair<>(Transformation.IDENTITY, new Point(1, 1))).toTessellation();
        ...
        new Pair<>(Transformation.ROT_90, new Point(0, 1))).toTessellation();
}
